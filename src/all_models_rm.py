...
def data_process_test(nn):
    ...
    if nn:
        y_np= to_categorical(y_np, num_classes=2)
        # y_train = to_categorical(y_train, num_classes=2)
        # y_test = to_categorical(y_test, num_classes=2)
    return X_np, y_np
    # return X_train, y_train, X_test, y_test

def data_process_train(nn):
    mats = []
    lst=[1,2,3,5]
    for i in lst:
        mats.append(scipy.io.loadmat(f'../p300_datasets/S{i}.mat'))
...
def model_xgb(X_train, y_train, X_test, y_test):
    ...
    # 计算每个通道的均值和方差
    mean_train = np.mean(X_train, axis=1)  # 计算所有样本和所有时间步上每个通道的均值，并转为一维数组
    var_train = np.var(X_train, axis=1) # 计算所有样本和所有时间步上每个通道的方差，并转为一维数组
    print("mean_train.shape", mean_train.shape)

    mean_test = np.mean(X_test, axis=1)  # 计算所有样本和所有时间步上每个通道的均值，并转为一维数组
    var_test = np.var(X_test, axis=1) # 计算所有样本和所有时间步上每个通道的方差，并转为一维数组

    X_train = X_train.reshape(X_train.shape[0], -1)
    X_test = X_test.reshape(X_test.shape[0], -1)

    # 将每个通道的均值和方差连接在二维数组的后面
    X_train = np.concatenate((X_train, mean_train, var_train), axis=1)
    X_test = np.concatenate((X_test, mean_test, var_test), axis=1)
...
if __name__ == "__main__":
    ...
    X_train, y_train = data_process_train(nn=True)
    X_test, y_test = data_process_test(nn=True)
    ...
    # X_train, y_train = data_process_train(nn=False)
    # X_test, y_test = data_process_test(nn=False)
